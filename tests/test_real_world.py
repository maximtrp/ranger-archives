...
from archives_utils import get_compression_command, get_decompression_command, find_binaries
...
class ArchiveTestRunner:
    ...
    def test_archive_format(self, archive_name: str, test_data_dir: Path,
        ...
        try:
            ...
            original_size = sum(f.stat().st_size for f in test_copy.rglob("*") if f.is_file())
            test_result["original_size"] = original_size

            # Get list of files to compress (relative paths)
            files_to_compress = [str(f.relative_to(test_copy)) for f in test_copy.rglob("*") if f.is_file()]

            # Test compression
            archive_path = work_dir / archive_name
            compression_command = get_compression_command(str(archive_path), flags, files_to_compress)

            if not compression_command:
                test_result["error"] = f"No compression command generated for {archive_name}"
                return test_result

            print(f"  Compressing with command: {' '.join(compression_command)}")

            start_time = time.time()
            success, stdout, stderr = self.run_command(compression_command, cwd=test_copy)
            ...
            test_result["compression_ratio"] = (1 - compressed_size / original_size) * 100 if original_size > 0 else 0

            # Test decompression
            decompression_command = get_decompression_command(str(archive_path), [], str(extract_dir))

            if not decompression_command:
                test_result["error"] = f"No decompression command generated for {archive_name}"
                return test_result
    ...
    def create_test_data(self):
        ...
        directories = [
            "small", "medium", "large", "nested/deep/structure"
        ]
        ...
        total_size = sum(f.stat().st_size for f in test_data_dir.rglob("*") if f.is_file())
    ...
    def _create_small_files(self, base_dir: Path):
        ...
        (base_dir / "small" / "small2.txt").write_text("Another small file with different content")

    def _create_medium_files(self, base_dir: Path):
        """Create medium test files"""
        print("  Creating medium files...")
        # Medium text file
        medium_text = '\n'.join(f'Line {i}: This is a medium-sized text file for testing compression algorithms.' for i in range(1000))
    ...
    def generate_report(self):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write("| Format | Status | Comp. Time | Decomp. Time | Original Size | Compressed Size | Ratio | Error |\n")
            f.write("|--------|--------|------------|--------------|---------------|-----------------|-------|---------|\n")
            ...
            for result in self.test_results:
                ...
                f.write(f"| {result['archive_format']} | {status} | "
    ...
    def _get_available_formats(self):
        ...
        for format_name, flags, required_tools in test_formats:
            ...
            if tools_available:
                ...
                print(f"  ✅ {format_name} - Tools available: {', '.join(required_tools)}")
            else:
                print(f"  ⚠️  {format_name} - Missing tools: {', '.join(required_tools)}")
    ...
    def _write_performance_stats(self, f, successful_results):
        ...
        performance_metrics = {
            "Best compression ratio": ("compression_ratio", lambda x: f"{x:.1f}% reduction", min),
...
def main():
    ...
    try:
        ...
        result_msg = {
            True: ("✅ All archive tests PASSED!", "The ranger-archives plugin is working correctly across all supported formats."),
            False: ("❌ Some archive tests FAILED!", "Check the detailed report for more information.")
        }[success]
